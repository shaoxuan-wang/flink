	...
	public void mergeNamespaces(N target, Collection<N> sources) throws Exception {
		...
		try {
			...
			for (N source : sources) {
				if (source != null) {
					writeKeyWithGroupAndNamespace(keyGroup, key, source);
			...
			if (current != null) {
				...
				byte[] targetKey = keySerializationStream.toByteArray();
				byte[] targetValueBytes = backend.db.get(columnFamily, targetKey);
				...
				keySerializationStream.reset();
				valueSerializer.serialize(current, keySerializationDateDataOutputView);
