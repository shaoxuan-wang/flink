...
/**
  * Created by wshaoxuan on 27/04/2017.
  */
class DStreamUDAGGITCase extends StreamingMultipleProgramsTestBase {
  ...
  def testUdaggSlidingWindowGroupedAggregate(): Unit = {
    ...
    val windowedTable = table
      ...
      .groupBy('w, 'string)
      .select('string, countFun('float), 'double.sum, weightAvgFun('long, 'int),
    ...
    val expected = Seq("Hello,2,3.0,1666,1", "Hi,2,11.0,5545,5", "Hello,4,14.0,5571,5",
  ...
  def testUdaggSessionWindowGroupedAggregate(): Unit = {
    ...
    val windowedTable = table
      ...
      .groupBy('w, 'string)
      .select('string, countFun('bigdec), 'float.sum, weightAvgWithMergeFun('long, 'int),
