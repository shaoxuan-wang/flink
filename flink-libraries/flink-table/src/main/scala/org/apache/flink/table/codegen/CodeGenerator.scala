...
class CodeGenerator(
  ...
  : GeneratedAggregationsFunction = {
    ...
    val accTypes = accTypeClasses.map { a =>
      a.getCanonicalName
    }
    ...
    val methodSignaturesList = aggFields.map {inFields =>
      val methodSignatures = for (f <- inFields) yield javaClasses(f)
      methodSignatures
    }
    ...
    aggregates.zipWithIndex.map {
      case (a, i) => {
        getUserDefinedMethod(a, "accumulate", Array(accTypeClasses(i)) ++ methodSignaturesList(i))
          .getOrElse(
            throw new CodeGenException(s"No matching accumulate method found for aggregate " +
        ...
        if (needRetract) {
          getUserDefinedMethod(a, "retract", Array(accTypeClasses(i)) ++ methodSignaturesList(i))
            .getOrElse(
              throw new CodeGenException(s"No matching retract method found for aggregate " +
                                           s"'${a.getClass.getCanonicalName}'.")
            )
        }
        ...
        if (needMerge) {
          ...
          if (methods.isEmpty || methods.length > 1) {
            throw new CodeGenException(s"No matching merge method found for aggregate " +
                                         s"${a.getClass.getCanonicalName}'.")
          }
        }
