...
/**
 * An interface for classes with a parameter specifying the column name of the predicted result.
 */
public interface HasPredResultColName<T> extends WithParams <T> {
	...
	ParamInfo <String> PRED_RESULT_COL_NAME = ParamInfoFactory
		...
		.setRequired()
		.setAlias(new String[] {"predColName"})
		.build();

	default String getPredResultColName() {
		return get(PRED_RESULT_COL_NAME);
	}
