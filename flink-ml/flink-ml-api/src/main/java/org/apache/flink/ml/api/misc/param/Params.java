...
public class Params implements Serializable {
	...
	public <V> V get(ParamInfo<V> info) {
		...
		return value == null ? info.getDefaultValue() : value;
	}
	...
	public String toJson() {
		...
		for (Map.Entry<String, Object> e : paramMap.entrySet()) {
			json.addProperty(e.getKey(), gson.toJson(e.getValue()));
		}
	...
	public void fromJson(String json, Map<String, Class<?>> classMap) {
		Gson gson = new Gson();
		classMap = classMap == null ? new HashMap<>() : classMap;
		JsonObject jobj = gson.fromJson(json, JsonObject.class);
		for (Map.Entry<String, JsonElement> e : jobj.entrySet()) {
			if (classMap.containsKey(e.getKey())) {
				paramMap.put(e.getKey(), gson.fromJson(e.getValue().getAsString(),
